...
public class SFTPCls {
	...
	public boolean connect(){
		try{
			log.info("SFTPCls - Start Establishing connection to - " + this.SFTPHOST);
	        if(this.privateKey != null)this.jSch.addIdentity(privateKey, USERPWS);
	        this.session = jSch.getSession(SFTPUSER,SFTPHOST,SFTPPORT);
			if(this.privateKey == null)this.session.setPassword(USERPWS);
	        java.util.Properties config = new java.util.Properties();
	        config.put("StrictHostKeyChecking", "no");
	        this.session.setConfig(config);
	        this.session.connect();
	        log.info("SFTPCls - Done Establishing connection to - " + this.SFTPHOST);
    ...
    public Set<String> sendFiles(String pathToCopyTo, String filePath, String fileNameRegex, String archPath) {
        ...
        try {
			...
			for(String fileToSend : getAllMacthingFilesLocal(filePath, fileNameRegex)){
		        ...
		        log.info("SFTPCls - File Send - START COPY FILE - " + fileToSend + " TO - " + pathToCopyTo + "/" + fileToSend);
	            channelSftp.put(new FileInputStream(fileToTrans), fileToTrans.getName());
		        log.info("SFTPCls - File Send - DONE COPY FILE - " + fileToSend + " TO - " + pathToCopyTo + "/" + fileToSend);
				fileListPass.add(pathToCopyTo + "/" + fileToSend);
				if(archPath != null){
					try{
						log.info("SFTPCls - File Send - MOVING FILE - " + fileToSend + " TO ARCHIVE - " + archPath);
    ...
    public Set<String> getFiles(String destPath, String filePath, String fileNameRegex, String archPath) {
	    ...
	    try {
			...
			for(String fileToGet : getAllMacthingFiles(filePath, fileNameRegex, channelSftp)){
		        log.info("SFTPCls - File Get - START COPY FILE - " + fileToGet + " TO - " + destPath + "/" + fileToGet);
				channelSftp.get(fileToGet, destPath + "/" + fileToGet); 
		        log.info("SFTPCls - File Get - DONE COPY FILE - " + fileToGet + " TO - " + destPath + "/" + fileToGet);
				fileListPass.add(destPath + "/" + fileToGet);
				if(archPath != null){
					log.info("SFTPCls - File Get - MOVING FILE - " + fileToGet + " TO ARCHIVE - " + archPath);
	...
	public void sftpChmode(int mode, String filePath, String fileNameRegex){
			Channel channel = null;
			ChannelSftp channelSftp = null;
	  	  try {
		        channel = session.openChannel("sftp");
		        ...
		        channelSftp = (ChannelSftp)channel;
				for(String fileToGet : getAllMacthingFiles(filePath, fileNameRegex, channelSftp)){
					channelSftp.chmod(mode, fileToGet);
				}
		    } catch (JSchException | SftpException e) {
		    	log.error(e.getMessage());
			}finally{
