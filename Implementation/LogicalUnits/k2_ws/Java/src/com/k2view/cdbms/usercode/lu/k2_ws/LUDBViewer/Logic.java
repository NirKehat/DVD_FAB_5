...
public class Logic extends WebServiceUserCode {
	...
	public static void wsLUDBViewer(String luName, String IID, String SyncMode) throws Exception {
		...
		try {
			...
			for(Object[] row : rs){
				if(row[0] != null && !"k2_ws".equals(row[0] + "") && !"k2_ref".equals(row[0] + ""))luListDD.append("<option value=\"" + row[0] + "\">" + row[0] + "</option>");
			}
		}finally {
	...
	private static Map<String, String> get_k2_objects_info(Map<String, String> k2ObjInfo) {
		ResultSetWrapper rs = null;
		try {
			rs = DBQuery("fabric", "SELECT object_name, end_sync_time, time_to_populate_in_sec FROM _k2_objects_info", null);
			for (Object[] row : rs) {
